...
class IVFC
{
    ...
    function __construct($superBlock)
    {
        ...
        $this->levels = array();
        $this->sboffset = unpack("Q", substr($superBlock, 0x88, 8))[1];
        for ($i = 0; $i < 6; $i++) {
            $sofs = 0x14 + ($i * (0x18));
            $tmplevel = new stdClass();
            $tmplevel->offset = unpack("Q", substr($superBlock, 0x88, 8))[1];
            $tmplevel->size = unpack("Q", substr($superBlock, $sofs + 8, 8))[1];
